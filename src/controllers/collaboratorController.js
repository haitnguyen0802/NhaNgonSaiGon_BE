...
exports.getCollaboratorById = async (req, res, next) => {
  try {
    ...
    const productCount = await Product.countDocuments({ collaborator: req.params.id });
...
exports.deleteCollaborator = async (req, res, next) => {
  try {
    ...
    if (productCount > 0) {
      return next(new AppError('Không thể xóa vì CTV đang quản lý sản phẩm. Vui lòng chuyển sản phẩm cho người khác trước.', 400));
    }
...
exports.getCollaboratorStats = async (req, res, next) => {
  try {
    ...
    // Tổng số sản phẩm
    const totalProducts = await Product.countDocuments({ collaborator: req.params.id });
